						...
						<view class="table-wrap">
							<table ref="table" border>
								<tr>
									<td 
										class="td"
										:class="{'active':subItem.sfpGameData[0]?.leftActive}"
									...
									<td 
										class="td"
										:class="{'active':subItem.sfpGameData[0]?.middleActive}"
									>
										<view class="td-wrap">
											<text class="label">平局</text>
											<text class="val">{{subItem.sfpGameData[0]?.drawOdds??'未受注'}}</text>
										</view>
									</td>
									<td
										class="td last"
										:class="{'active':subItem.sfpGameData[0]?.rightActive}"
									>
										<view class="td-wrap">
		...
		onLoad(options) {
			...
			detailOptions.lotteryOrderOdds.map(item=>{
				...
				try{
					...
					const isHomeIn = item.sfpData.homeOdds===item.sfpGameData[0].homeOdds && item.sfpData.homeOddsName===item.sfpGameData[0].homeOddsName
					const isAwayIn = item.sfpData.awayOdds===item.sfpGameData[0].awayOdds && item.sfpData.awayOddsName===item.sfpGameData[0].awayOddsName
					const isDrawIn = item.sfpData.drawOdds===item.sfpGameData[0].drawOdds && item.sfpData.drawOddsName===item.sfpGameData[0].drawOddsName
					if(isHomeIn){
						item.sfpGameData[0].leftActive=true
					}
					if(isAwayIn){
						item.sfpGameData[0].rightActive=true
					}
					if(isDrawIn){
						item.sfpGameData[0].middleActive=true
					}
				}catch(e){
					//TODO handle the exception
				}
