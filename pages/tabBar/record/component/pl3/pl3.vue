<template>
	<view style="padding-bottom:200px;">
		...
		<uni-card title="方案详情" :is-shadow="false" style="margin: 30rpx 0 0 0;padding:0;">
			...
			<view class="content">
				<view class="inner">
					<view class="inner-item" v-for="(item,index) in record.lotteryOrderOdds">
						<view class="inner-flex" v-if="item.permutationThreeType===1 || item.permutationThreeType===8">
							<view style="margin-right:10px;">直选普通</view>
							<view class="circle-wrap" v-if="item.permutationNum.one">
							...
							<view class="circle-wrap" v-if="item.permutationThreeType===8 && item.permutationNum.num">
								<text class="circle" v-for="subItem in item.permutationNum.num">{{subItem}}</text>
							</view>
						</view>
						<view class="inner-flex" v-if="item.permutationThreeType===2">
							<view style="margin-right:10px;">直选和值</view>
							<view class="circle-wrap">
								<text class="circle" v-for="subItem in item.permutationNum.sumValue">
								{{subItem}}
								</text>
							</view>
						</view>
						<view class="inner-flex" v-if="item.permutationThreeType===3">
							<view style="margin-right:10px;">组3</view>
							<view class="circle-wrap">
								<view v-if="item.permutationNum.fixed && item.permutationNum.fixed.length>0" style="font-size:12px;color:#999;">[胆:</view>
								<text class="circle" v-for="subItem in item.permutationNum.fixed">
								{{subItem}}
								</text>
								<view v-if="item.permutationNum.fixed && item.permutationNum.fixed.length>0" style="font-size:12px;color:#999;">]</view>
								<text class="circle" v-for="subItem in _.difference(item.permutationNum.num,item.permutationNum.fixed)">{{subItem}}</text>
							</view>
						</view>
						<view class="inner-flex" v-if="item.permutationThreeType===4">
							<view style="margin-right:10px;">组6</view>
							<view class="circle-wrap">
								<view v-if="item.permutationNum.fixed && item.permutationNum.fixed.length>0" style="font-size:12px;color:#999;">[胆:</view>
								<text class="circle" v-for="subItem in item.permutationNum.fixed">
								{{subItem}}
						...
						<view class="inner-flex" v-if="item.permutationThreeType===5">
							<view style="margin-right:10px;">组3组6</view>
							<view class="circle-wrap">
								<view v-if="item.permutationNum.fixed && item.permutationNum.fixed.length>0" style="font-size:12px;color:#999;">[胆:</view>
								<text class="circle" v-for="subItem in item.permutationNum.fixed">
								{{subItem}}
						...
						<view class="inner-flex" v-if="item.permutationThreeType===6">
							<view style="margin-right:10px;">组选和值</view>
							<view class="circle-wrap">
								<text class="circle" v-for="subItem in item.permutationNum.num">
								{{subItem}}
								</text>
							</view>
						</view>
						<view class="inner-flex" v-if="item.permutationThreeType===7">
							<view style="margin-right:10px;">组选单式</view>
							<view class="circle-wrap">
								<text class="circle" v-for="subItem in item.permutationNum.doubleNum">
								{{subItem}}
								</text>
								<text class="circle" v-for="subItem in item.permutationNum.singleNum">
								{{subItem}}
								</text>
							</view>
						</view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			getApp().globalData.detailOptions = null
