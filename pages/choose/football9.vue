		...
		<view class="nav-bar">
			...
			<uni-nav-bar left-icon="left" background-color="#cc0002" color="#ffffff" height="40px"
				...
				<view class="title-center">
					<view>
						<text class="main-tit">足球任选9-<text
		...
		<scroll-view scroll-y="true" :style="{paddingTop:(iStatusBarHeight+60)+'px',paddingBottom:'100px'}">
			...
			<view class="content" v-for="(item,key) in list.match_list" :key="key">
				<view class="content-item">
					...
					<view :class="{'active':item.isDan}" @click="danClick(item,key)" class="tag">胆</view>
				</view>
			</view>
		</scroll-view>
		<view class="opt-wrap">
			<view class="select-tip" v-if="selectCount<9">
				<text class="title">已选<text class="red">{{selectCount}}</text>场，还需至少选择<text
						class="red">{{9-selectCount}}</text>场</text>
				<text @click="goExplain" style="text-decoration: underline;">{{noSelectDescTip}}</text>
			</view>
			<view v-if="selectCount>=9">
				<view class='select-info'>
					<text v-if="selectCount<=9" style="margin-left:15px;">已选 <text class="red">{{ selectCount }}</text>
						场</text>
					<text v-if="selectCount>9" style="margin-left:15px;"><text class="red">{{ selectCount }}</text>
						转<text class="red">9</text>场</text>
					<view class="select-amount">
						<text v-if="danIndexes>0" class="red">{{danIndexes}}</text>
						<text style="margin-right:10px;" v-if="danIndexes>0">胆</text>
						<text class="amount-tit">金额<text>{{amountValue}}</text>元</text>
					</view>
				</view>
				<view class="btns">
					<view class="btns-item" style="background-color: #fff;">
						<view class="scale-block">
							<uni-number-box :min="1" :max="50000" :width="60" :value="scaleValue"
								@change="changeNumBox" />
							<text>倍</text>
						</view>
					</view>
					<view style="flex:1"></view>
					<view @click="submitOrder(3)" class="btns-item" style="width:35px;">保存</view>
	...
	export default {
		...
		computed: {
			...
			danIndexes() {
				...
				for (let key in obj) {
					if (this.list.match_list[key].isDan) {
						count++
					}
				}
				return count
			...
			amountValue() {
				...
				const obj = _.groupBy(this.selectList, (o) => o.key)
				for (let key in obj) {
					matchOptions.push(obj[key])
					if (this.list.match_list[key].isDan) {
						danIndexes.push(key - 1)
					}
				}
				const result = calculateBettingAmount(danIndexes, matchOptions)
				return result
			},
			endTime() {
		...
		methods: {
			select(item){
				if("玩法说明" === item.text){
					uni.navigateTo({
						url: '/pages/choose/component/football9/description/description'
					})
				}else if("即时比分" === item.text){
					uni.navigateTo({
						url: '/pages/choose/component/football9/score/score'
					})
					...
					uni.navigateTo({
						url: '/pages/choose/component/football9/lotteryResult/lotteryResult'
					})
				}
			},
			danClick(item,key) {
				const isSelect = this.selectList.filter(o=>o.key===key).length>0
				if(isSelect){
					if(this.danKeys.length===8 && !item.isDan){
						uni.showToast({
							icon:'none',
							title:'最多只能设置8个胆'
						})
			...
			async submitOrder(orderType) {
				...
				for (let key in selectListObj) {
					...
					//原始数据
					let sfpGameData = {}
					const itemObj = this.list.match_list[key]
					sfpGameData['homeOddsName'] = '胜'
					sfpGameData['homeOdds'] = itemObj.odds[3]
					sfpGameData['drawOddsName'] = '平'
					sfpGameData['drawOdds'] = itemObj.odds[1]
					sfpGameData['awayOddsName'] = '负'
					sfpGameData['awayOdds'] = itemObj.odds[0]
					sfpGameData.key = key
				...
				if(orderType===3){
					...
					getApp().globalData.detailOptions = { ...options,danKeys:this.danKeys, topBonusAmount,maskLotteryId };
					uni.navigateTo({
						url:'/pages/tabBar/record/component/football9/football9'
					})
				}else{
					getApp().globalData.detailOptions= {...options,danKeys:this.danKeys}
					uni.navigateTo({
